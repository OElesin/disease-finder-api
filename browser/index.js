async function runExample() {
  ...
  const imageData = await imageLoader.getImageData('./test-image.jpg');
...
/**
 * Render Prediction output to Html.
 */
function printMatches(data) {
  ...
  if (!data || data.length === 0) {
    ...
    outputClasses = imagenetClassesTopK(data, 1);
  }
  const predictions = document.getElementById('predictions');
  predictions.innerHTML = `Predicted class: ${outputClasses}`;
}